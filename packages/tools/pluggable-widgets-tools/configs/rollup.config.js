...
const widgetNsDir = join(widgetPackage.replace(/\./g, "/"), widgetName.toLowerCase());
const outWidgetFile = join(widgetNsDir, `${widgetName}`);
...
const createCssUrlTransform = production => {
    const prodTransform = asset => `${widgetNsDir}/${asset.url}`;
    const devTransform = asset => `/widgets/${widgetNsDir}/${asset.url}`;
